...
.content {
 ...
 <?php
            ...
            $resultado = mysqli_query($conexao, "select * from lote_vacina where idlote_vacina = $cod");

            $medic = mysqli_query ($conexao, "select * from vacinas");

            $mostra = mysqli_fetch_array($resultado);
            
            $cod_lote = $mostra['idlote_vacina'];

            $cod_vac = $mostra['cod_vac'];
...
<body style="background-color: #F0FFF0;">
        ...
        <form method="POST" action="../php/phpeditavac.php" enctype="multipart/form-data">
      <br><br><br>
    ...
    <div class="container">
        ...
        <option> Escolher...</option>
             ...
             while($registro = mysqli_fetch_assoc($medic)) { 
                 ...
                 if($registro['cod_vac'] == $mostra['cod_vac']){
                 ?>
                    <option  value = "<?php echo $registro['cod_vac'] ?>" selected> <?php echo $registro['nome'] ?> </option>
                 <?php }else{ ?>
